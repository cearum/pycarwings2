...
class Leaf:
	...
	def get_status_from_lat_long_update(self, result_key):
		response = self.session._request_with_retry("MyCarFinderResultRequest.php", {
			...
			"resultKey": result_key,
                ...
                print ("Status Lat Long Result: ", response)
		# responseFlag will be "1" if a response has been returned; "0" otherwise
		if response["responseFlag"] == "1":
			return CarwingsLocationStatusResponse(response)

		return None
                
        def get_vehicle_lat_long(self):	
		response = self.session._request_with_retry("MyCarFinderLatLng.php", {
			"RegionCode": self.session.region_code,
			"lg": self.session.language,
			"DCMID": self.session.dcm_id,
			"VIN": self.vin
		...
		if response["status"] == 200:
			...
			#print("long: ", response["lng"])			
			return CarwingsLocationResponse(response)
